...
GoogleGeocoder.prototype.request = function(location, sensor, bounds) {
  ...
  if ( typeof bounds !== 'undefined' && bounds instanceof Array ) {
  	if ( bounds[0] instanceof Array && bounds[1] instanceof Array ) {
  		path += '&bounds=' bounds[0][0] +
  			',' + bounds[0][1] +
  			'|' + bounds[1][0] +
  			',' + bounds[1][1];
  	}
...
// Get Geocode
Geo.prototype.geocoder = function(geocoder, location, sensor, bounds, callback, timeout) {
	// Check Input
	...
	if ( sensor == null || location === undefined ) {
		sensor = false;
	}
	if ( callback == null ) {
	...
	var options = geocoder.request(location, sensor, bounds);
...
Geo.prototype.geomodel = function(modelInstance, locationGetterCallback, geohashSetterCallback, callback) {
	...
	Geo.prototype.geocoder(new GoogleGeocoder(), locationGetterCallback(modelInstance), false, null, function(formattedAddress, latitude, longitude) {
